...
interface EditableContentProps {
  ...
  className?: string;
  onUpdate?: (newContent: { en: string; bg: string }) => void;
}
...
const EditableContent: React.FC<EditableContentProps> = ({ 
  ...
  const [editedContent, setEditedContent] = useState<{ en: string; bg: string }>(content);
  ...
  if (isEditing && isAdmin) {
    return (
      <div ref={editRef} className="relative border border-primary p-2 rounded-md">
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-medium mb-1">English</label>
            <ContentEditor 
              initialValue={editedContent.en} 
              type={type}
          ...
          <div>
            ...
            <ContentEditor 
              initialValue={editedContent.bg} 
              type={type}
              onChange={(value) => handleContentChange("bg", value)}
            />
          </div>
        </div>
  ...
  return (
    <div className={`group relative ${className}`} id={id}>
      {type === "heading" ? (
        <h2 className="text-2xl font-bold">{displayContent}</h2>
      ) : (
        <p>{displayContent}</p>
      )}
...
const ContentEditor: React.FC<{
  ...
  return (
    <textarea
      className="w-full px-3 py-2 border border-gray-200 rounded-md resize-y"
      value={initialValue}
      onChange={(e) => onChange(e.target.value)}
      rows={3}
      autoFocus
